...
import { TProdutItem } from '@/constants/types'
...
const ProductPage: FC<Props> = ({ product }) => {
	...
	const isOutOfStock = selectedVariation ? parseInt(selectedVariation.stock) <= 0 : true

	useEffect(() => {
		if (product && product.variations.length > 0 && !selectedVariation) {
			const firstAvailable = product.variations.find(v => parseInt(v.stock) > 0) || product.variations[0]
						...
						<div className="flex flex-col gap-4">
							<div className="flex items-center justify-between">
								...
								<QuantitySelector
									...
									maxQuantity={selectedVariation ? parseInt(selectedVariation.stock) : 0}
								/>
							</div>
							<Button
								color="primary"
