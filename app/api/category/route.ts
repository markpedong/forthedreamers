...
const save = async (request: NextRequest, editing: boolean) => {
  ...
  if (!parsed.success || (editing && !parsed.data.id)) return NextResponse.json({success: false, message: 'Invalid category'}, {status: 400})
  try {
    return NextResponse.json({success: true, message: 'Saved successfully', data: await saveCategory(parsed.data.name, parsed.data.id)})
  } catch (error) {
    return NextResponse.json({success: false, message: error instanceof Error ? error.message : 'Unable to save category'}, {status: 400})
  }
}
