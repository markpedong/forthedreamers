...
export const POST = async (request: NextRequest) => {
  ...
  if (!parsed.success) return NextResponse.json({success: false, message: 'Check your address details'}, {status: 400})
...
export const PUT = async (request: NextRequest) => {
  ...
  try {
    ...
    return NextResponse.json({success: false, message: error instanceof Error ? error.message : 'Unable to update address'}, {status: 400})
  }
}
...
export const DELETE = async (request: NextRequest) => {
  ...
  try {
    ...
    return NextResponse.json({success: false, message: error instanceof Error ? error.message : 'Unable to delete address'}, {status: 400})
  }
}
