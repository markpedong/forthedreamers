...
export async function POST_VALIDATE(request: NextRequest) {
  try {
    ...
    // Calculate discount
    const discountValue = coupon.type === 'percentage' ? orderTotal * (coupon.value / 100) : coupon.value;

    const finalDiscount = coupon.maxDiscount ? Math.min(discountValue, coupon.maxDiscount) : discountValue;
