...
const knownErrors = ['Product not found', 'A paid purchase is required to review this product', 'You have already reviewed this product']

export const GET = async (request: NextRequest, {params}: {params: Promise<{slug: string}>}) => {
  const page = z.coerce.number().int().min(1).max(10000).safeParse(request.nextUrl.searchParams.get('page') ?? 1)
  const limit = z.coerce.number().int().min(1).max(20).safeParse(request.nextUrl.searchParams.get('limit') ?? 6)
...
export const POST = async (request: NextRequest, {params}: {params: Promise<{slug: string}>}) => {
  ...
  try {
    ...
    const message = error instanceof Error && knownErrors.includes(error.message) ? error.message : 'Unable to submit review'
