...
const save = async (request: NextRequest, editing: boolean) => {
  ...
  if (!parsed.success || (editing && !parsed.data.id)) return NextResponse.json({success: false, message: 'Invalid product'}, {status: 400})
  try {
    return NextResponse.json({success: true, message: 'Saved successfully', data: await saveProduct(parsed.data, editing)})
  } catch (error) {
    return NextResponse.json({success: false, message: error instanceof Error ? error.message : 'Unable to save product'}, {status: 400})
  }
}
...
export const DELETE = async (request: NextRequest) => {
  ...
  try {
    ...
    return NextResponse.json({success: false, message: error instanceof Error ? error.message : 'Unable to delete product'}, {status: 400})
  }
}
