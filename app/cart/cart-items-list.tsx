...
const CartItemsList = ({ items }: { items: CartItem[] }) => {
  ...
  const applyOptimisticChange = (id: string, quantity?: number) => {
    setVisibleItems(state =>
      quantity === undefined ? state.filter(item => item.id !== id) : state.map(item => (item.id === id ? { ...item, quantity } : item))
    )
  }
  ...
  const total = visibleItems.reduce((sum, item) => sum + (item.variant.discountedPrice ?? item.variant.price) * item.quantity, 0)
        ...
        {visibleItems.map(item => {
          ...
          return (
            <div key={item.id} className='flex gap-4 p-4 border rounded-lg bg-card'>
              {item.variant.image && (
                <Image src={item.variant.image} alt={item.variant.product.name} width={80} height={80} className='rounded object-cover' />
              )}
              <div className='flex-1'>
                <Link href={`/products/${item.variant.product.slug}` as never} className='font-semibold hover:underline'>
