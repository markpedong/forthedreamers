...
assert.match(checkoutPage, /shipments: \[\.\.\.sellerGroups\.keys\(\)\]/);
assert.match(checkoutPage, /PAYMENT_METHODS\.map/);
assert.match(checkoutPage, /Coming soon/);
assert.match(checkoutRoute, /checkoutSchema\.safeParse/);
assert.match(checkoutService, /input\.paymentMethod !== ENABLED_PAYMENT_METHOD/);
assert.match(checkoutService, /submittedShipments\.has/);
assert.match(checkoutService, /shippingBySeller/);
assert.match(checkoutService, /getCourier\(selectedCode\)/);
assert.doesNotMatch(checkoutService, /shippingMethod\.price/);
assert.match(read('lib/services/seller.ts'), /\.min\(1, 'Select at least one courier'\)/);
