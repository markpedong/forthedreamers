...
export async function cached<T>(scope: string, key: string, ttl: number, query: () => Promise<T>): Promise<T> {
  ...
  try { await redis('SET', fullKey, JSON.stringify(value), 'EX', ttl); } catch { /* Database result remains usable. */ }
  return value;
}
export async function invalidateCatalog() {
  ...
  try { await redis('SET', cacheKeys.generation('catalog'), crypto.randomUUID()); }
  catch { console.warn('Catalog cache invalidation unavailable; entries expire by TTL'); }
}
