...
export const cartSelect = {
  ...
  variant: { select: {
    id: true, name: true, price: true, discountedPrice: true, stock: true, image: true,
    product: { select: { id: true, name: true, slug: true } },
  } },
} satisfies Prisma.CartItemSelect;
export type CartItem = Prisma.CartItemGetPayload<{ select: typeof cartSelect }>;
export const readCart = (userId: string) => prisma.cartItem.findMany({ where: { userId }, select: cartSelect, orderBy: { createdAt: 'asc' } });
...
export async function mutateCart(userId: string, operation: 'add' | 'update' | 'remove', id: string, quantity = 1) {
  if (typeof id !== 'string' || !id || id.length > 100 || !Number.isSafeInteger(quantity) || quantity < 1 || quantity > 999) throw new CartError('Invalid cart input');
  // Serializable retries protect cumulative quantity checks against concurrent tabs.
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      return await prisma.$transaction(async tx => {
        const existing = operation === 'add'
        ...
        if (operation === 'remove') {
          ...
          const variant = await tx.variant.findUnique({ where: { id: operation === 'add' ? id : existing!.variantId }, select: { id: true, productId: true, stock: true, product: { select: { status: true } } } });
          ...
          item = existing
            ? await tx.cartItem.update({ where: { id: existing.id, userId }, data: { quantity: nextQuantity }, select: cartSelect })
            : await tx.cartItem.create({ data: { userId, variantId: variant.id, productId: variant.productId, quantity: nextQuantity }, select: cartSelect });
        }
        return { item, removedId: operation === 'remove' ? id : null, count: await tx.cartItem.count({ where: { userId } }) };
