...
export const sellerSignup = async ({storeName, name, email, password}: z.infer<typeof sellerSignupSchema>) => {
  if (await prisma.seller.findUnique({where: {storeName}, select: {id: true}})) throw new Error('Store name is already taken')
  ...
  if (error || !authUser || !authUser.identities?.length) throw new Error('Unable to sign up. Sign in if you already have an account.')
  try {
    await prisma.$transaction(async tx => {
      await tx.user.upsert({
        where: {id: authUser.id},
        update: {email, name, image, role: USER_ROLE.SELLER},
        create: {id: authUser.id, email, name, image, emailVerified: Boolean(authUser.email_confirmed_at), role: USER_ROLE.SELLER}
      })
      await tx.seller.create({data: {storeName, userId: authUser.id}})
    })
  } catch {
    throw new Error('Account created, but the store could not be created. Verify your email and sign in to finish seller setup.')
  }
