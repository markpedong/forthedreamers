...
export const saveProduct = async (input: unknown, editing: boolean) => {
  ...
  const result = await prisma.$transaction(async tx => {
    ...
    await tx.variant.deleteMany({where: {productId: id, id: {notIn: variants.flatMap(variant => variant.id ? [variant.id] : [])}}})
    await tx.spec.deleteMany({where: {productId: id, id: {notIn: specs.flatMap(spec => spec.id ? [spec.id] : [])}}})
