...
type ReviewQuery = {page: number; limit: number; rating?: number; sortBy: 'rating' | 'createdAt'; order: 'asc' | 'desc'}
  ...
  const [reviews, total, aggregate] = await Promise.all([
    prisma.review.findMany({
      ...
      select: {
        ...
        user: {select: {name: true, image: true}}, variant: {select: {name: true}}
      },
  ...
  const review = await prisma.$transaction(async tx => {
    ...
    const purchased = await tx.orderItem.findFirst({
      where: {
        ...
        order: {userId, status: {in: [...paidStatuses]}, OR: [{orderGroupId: null}, {orderGroup: {paymentStatus: 'PAID'}}]},
    ...
    await tx.product.update({where: {id: product.id}, data: {rating: aggregate._avg.rating ?? 0, reviewCount: aggregate._count._all}})
    return created
