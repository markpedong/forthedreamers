...
export const getUserAddresses = (userId: string) => prisma.address.findMany({
  where: {userId},
  orderBy: [{isDefault: 'desc'}, {updatedAt: 'desc'}],
  omit: {createdAt: true, updatedAt: true, userId: true}
})

export const createAddress = (userId: string, values: AddressInput) => prisma.$transaction(async tx => {
...
export const updateAddress = (userId: string, values: AddressUpdateInput) => prisma.$transaction(async tx => {
...
export const deleteAddress = (userId: string, id: string) => prisma.$transaction(async tx => {
...
export const setDefaultAddress = (userId: string, id: string) => prisma.$transaction(async tx => {
  const existing = await tx.address.findFirst({where: {id, userId}})
  if (!existing) throw new Error('Address not found or unauthorized')
  await tx.address.updateMany({where: {userId, isDefault: true, id: {not: id}}, data: {isDefault: false}})
  return tx.address.update({where: {id}, data: {isDefault: true}, omit: {createdAt: true, updatedAt: true, userId: true}})
})

export const getUserAccounts = (userId: string) => prisma.account.findMany({
  where: {userId},
