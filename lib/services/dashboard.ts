...
const normalizeDashboardRange = (value?: string): DashboardRange => DASHBOARD_RANGES.find(range => range === value) ?? '30d'
...
const getSellerInventory = async (sellerId: string) => {
  ...
  const [summary, attention] = await Promise.all([
    ...
    `),
    prisma.$queryRaw<Array<{id: string; name: string; image: string | null; stock: number; status: PRODUCT_STATUS}>>(Prisma.sql`
...
const requireSeller = async () => {
  ...
  if (!user || !user.emailVerified || user.banned || user.role !== USER_ROLE.SELLER) throw new Error('Seller dashboard access required')
...
export const getAdminDashboardData = async (range: DashboardRange): Promise<AdminDashboardData> => {
  const user = await getSessionUser()
  if (!user || !user.emailVerified || user.banned || user.role !== USER_ROLE.ADMIN) throw new Error('Administrator dashboard access required')
